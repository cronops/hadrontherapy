...
The most aparent one being the missing forward yield of fragments. Further work is needed to determine the cause for these insufficient yields. As a good starting hypothesis one may choose to study the isotropicity of the evaporation model, as the missing yields at low angles are especially apparent for light fragments.

For further information the simulation may be altered so that the detection-voxels in the phantom are changed to the \textit{G4SensitiveDetector} type from \textit{G4PrimitiveDetector}. This change will allow better analysis of what happens inside the water phantom instead of just the analysis of the resulting fragment distributions that was the focus of this work.

Thus, I hope through this work to provide a contribution to the pool of carbon treatment related simulation analyses, and ease the development of models.

The experimental data pool of carbon beams is rather limited. The paper by E.Haettner used as reference in this article is one of the more thorough ones with relatively good possibilities for recreating the experiment for data-comparison. However, as benchmarking was not the primary reason for creating the used GSI data set there remains a number of things in the experimental data that were not recreatable in the simulation code.

I also studied others models for hadronic simulations. From this I concluded there is reason to believe the problems encountered with the binary model could be solved with the INCL and ABLA models. However, the anticipated release of the INCL code supporting light-nucleus interactions has not taken place at the time of writing. Therefore this work is left for the future. However, this paper provides an adaptable Geant4 framework and related ROOT analysis scripts that may be used together with any Geant4 compatible hadron model. The theoretical bases of the INCL and ABLA codes are introduced in this paper in appendix~\ref{appendixincltheory}.
