...
For light nuclei the excitation energy is often of the magnitude of the nucleon binding energy. Therefore it is possible to construct rather simple predictive statistical models for break-up. This approach was first introduced by Enrico Fermi and is thus called the Fermi break-up model. It is applied to all nucleuses with $A < 17$ that have a high enough excitation energy. Fermi break-up can be seen as a extreme case of fissioning in the nucleus where the nucleus compeltely ``explodes``. %fixme: phase space should be mentioned
\clearpage
...
\item The geometry had to be rewritten, but in such a way that it may be interchangeable through macro commands. Reqriting the geometry also meant having to rewrite some of the detection code.
\item Multiple runs needed to be able to be run.
...
Reference data for the measurements involved in this paper have been acquired from E. Haettner ~\cite{ehaettner}  Haettner performed these measurements at the GSI facility in Darmstadt, Germany. Haettner used two different setups for the measurements made with the H1 detector and the MWPC detector used for calculating the Bragg-curve. In the following parts of this paper we will discuss approaches to reproduce these results through simulation.

\subsubsection{H1 measurements}

The figure ~\cite{fig:haettnersetup3} shows the geometry Haettner used in angular and energy distribution measurements with the H1 detector. These ''H1 measurement`` setup was recreated in the simulation environment. The major differences being:
\begin{itemize}
\item the pipe in the water-phantom was by inspection of photograph \cite{fig:haettnersetup2} deemed impossible to recreate in such a way that it could be assumed to recreate any backscattering effects or similar induced by the pipe. Thus a simple cube-shaped phantom was used.
...
Through personal correspondence with E.Haettner it was found out that the . Thus the measurement points for the thickness of the water-phantom had to be reduced by ... as per ~\cite[...]{ehaettner}. Further ehaettner had treated the data with a constant at the magnitude of... however the reason for this was unknown.

\subsubsection{Bragg-Curve measurements}
In this paper the bragg-curve is calculated by imaginary voxel-layers in the phantom, and therefore is only for interest in compensating the water equivalents added by Haettner into the results for the different parts of the experiment.
...
0m &4x4 mm& 3.9 mm & 3http://www.peppar.fi/.9 mm\\
...
E. Haettner has provided estimates for the yields inside 10 degrees angle. This estimate is based on fitting the angular distributions with a normal distribution to low angles of the angular distribution and an exponential function to the larger angles. This yielded relatively large error margins for Haettner's results. Comparable results should be producable by picking all events within a circle with radius $r = \text{Scattering distance} \times \tan(10^{\circ} \text{degrees}) $ of the center of the scoring plane.

The results are gathered in Appendix B~\ref{AppendixB}. It is aparent the match is rather good for most fragments, while outside all margins of error for both Lithium and Helium for almost all phantom depths.
...
\section*{Appendix B \label{AppendixB}: Fragment yields for 0$^{\circ}$-10$^{\circ}$ angles}
...
\addcontentsline{toc}{section}{Appendix D: Log of work}
