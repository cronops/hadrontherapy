...
Detta kandidatarbete utforskare med hjälp av Geant4 simulationsgränssnittet simulerandet av kolkärnor som skjuts in i en vattenfantom. Motiveringen till dessa simulationer är  den medicinska tillämpningen av Monte Carlo kod för tungionbehandlingar. Geant4 är ett mångbrukligt fysiksimulationsprogram som är utvecklat vid europeiska organisationen för kärnforskning (CERN). Geant4 erbjuder flera olika kompatibla modeller för hadronisk fysik, för detta arbete har vi valt att bruka Binary kaskadmodellen som är utvecklad som ett internationellt samarbete mellan CERN och ett antal internationella forskningsinstitutioner. Resultaten från simulationerna jämförs med experimentell data från E.~Haettners examensarbete. Haettner utförde mätningarna vid GSI Darmstadt år 2006. I arbetet har hadrontherapy-exempelkoden i Geant4 vidareutvecklats. Med hjälp av denna kod har vi sedan kunnat modelera och estimera fördelningarna och magnituderna av de uppkomna fragmenten i i slutändan av den primära strålens räckvidd. Denna studie är del av den internationella atomenergiorganisationens (IAEA) program för att ihopställa experimentell kärnfysikdata och därtill relaterade Monte Carlo koder och simulationer för tungionbehandlingar.
\end{abstractpage}
...
\clearpage
\section{Simulation setup\label{SimulationSetup}} %practical aspects of simulation
...
Ioniserande strålning har brukats i medicinskt terapisyfte i närmare hundra år. De tidiga strålterapierna var dock mycket rudimentära och var ofta skadliga för patienten. Tekniken har sedan dess förbättrats avsevärt och man känner till dosdepositionsmekanismerna väl, men fysikens lagar ställer en gräns för hur noga elektromagnetisk strålning kan deponeras i den sjuka vävnaden. Sedan 1960-talet har man försökt lösa denna problemställningen med hadronterapier. Hadroner, som är massiva partiklar uppbyggda av kvarkar, växelverkar med vävnad på ett mycket annorlunda sätt. Hadroner deponerar merparten av sin ioniserande verkan på ett mycket kortare område än den exponentiellt avtagande elektromagnetiska strålningen, såsom framgår i graf~\ref{fig:DosesFromHaettnerSvenska}. Detta arbete koncentrerar sig specifikt på simulation av kolkärnor, en ovanligare variant av hadronterapi med vissa ytterligare för- nackdelar jämfört med mera konventionell protonterapi.

\begin{figure}[!h]
\begin{center}
\includegraphics[width=0.8\textwidth]{images/DosesFromHaettner.png}  
\caption{\label{fig:DosesFromHaettnerSvenska}Relativa ionisationsdosen som funktion av djupet i vatten från tre olika slags strålar. Blå konventionell elektromagnetisk strålning, röd protonstrålning samt grön kolkärnestråle. Data och bild från Weber~\cite{weber} och Haettner~\cite{ehaettner}.} 
\end{center}
\end{figure} 

Mekanismerna i dessa terapier är svårare behärskade än i konventionell strålterapi. Exempelvis efekten av fragmenteringen av den primära strålen till mindre fragment måste tas i hänsyn. Därför är det speciellt viktigt att känna till tillförlitligheten på de simulationsmodeller som finns till förfogande. Syftet med detta arbete är att producera och utvärdera en Monte Carlo simulation i simulationsgränssnittet Geant4 motsvarande experimentell data samlad av E.~Haettner år 2006 vid GSI i Darmstadt, Tyskland. Haettner utförde mätningar där en kolkärnestråle skjöts in i en vattenfantom. Haettners kolkärnestråle hade en energi på 400~MeV/u som är ett övre maximum för energier som används inom medicinska behandlingar.

Detta arbete är en del av IAEA/INDC projektet ''Heavy Charged-Particle Interaction Data For Radiotherapy`` och arbetet har producerat en utgångspunkt för andra liknande jämförelser inom projektets ramar. För arbetet fanns som bas till förfogande en simulation för protonbehandling av ögonbottenscancer utvecklad vid INFN Catania i Italien. Simulationen modifierades enligt beskrivningen i kapitel~\ref{SimulationSetup} för att motsvara GSIs experimentella uppsättning.

För arbetets del var den intressantaste simulationsmodellerna de koder som var inblandade i de nukleära processerna och därmed fragmentationen. För de intranukleära processerna användes Binary cascade kaskadmodellen, och för de påföljande evaporation- och fissionskedena användes Geant4 evaporation- och fissionmodellerna. Intranukleära kaskadmodeller såsom binary cascade baserar sig på fenomenologi och modelerar mycket kvicka processer, medan evaporationsprocesserna bestäms till större del ur experimentella statistiska fördelningar och beskriver processer som sker under storleksordningar längre tidsintervall.

I arbetet var en av utmaningarna att återskapa E.~Haettners experiment möjligast väl i en efektiv simulation. I senare delen av kapitel~\ref{SimulationSetup} beskrivs vilka förenklingar som kunde göras och hur symetri kunde brukas för att göra simulationen kvickare och efektivare men samtidigt producera ekvivalenta resultat.
\begin{figure}[!h] 
\begin{center}
\includegraphics[width=0.8\textwidth]{images/plots/fragmentEnergy/fragmentEnergyDistr.png}  
\caption{\label{fragmentEnergySvenska}Fragmentens energifördelning i Geant4 simulation med 27.1~cm tjock vattenfantom. Hela linjer från simulation, trianglar experimentell data. Resultaten har normaliserats till mängden incidentpartiklar och detektorns rymdvinkel. Även i denna graf ser man en ansenlig del av Heliumfragmenten saknas, fastän energifördelningen är i samklang med experimentella resultat.}
\end{center}
\end{figure}

Då resultaten analyserades i ROOT-programmet visade sig flera anmärkningsvärda resultat. I största allmänhet var resultaten i god samklang med experimentell data, men med vissa markanta undantag. Ett av dessa undantag var sättet som de bildade fragmenten skingrats i olika vinklar. Speciellt för de lättare fragmenten fanns en markant skillnad mellan experimentella resultat och simulationen i området $[0,3^\circ]$. Däremot var fragmentens energifördelning (bild~\ref{fragmentEnergySvenska}) var väl i linje med de experimentella resultaten. Även totala mängden fragment var för de flesta isotoper mycket lika de experimentellt aproximerade mängderna. Undantaget i fragmentmängderna var dock helium som bildades i klart mindre mängder än man kunnat förvänta sig utgående från experimentell data.

Arbetet visade att man med hadroniska Monte Carlo modeller nog kan få meningsfulla resultat till forskning inom tungionsterapier, men att mycket arbete för att utforska modellernas svaga sidor finns att utföra. Detta arbete ger även en god bas för andra liknande analyser av simulationskoder som är kompatibla med Geant4 gränssnittet. En av dessa koder som jag förväntar mig uppnå bättre resultat är INCL kaskaden med ABLA evaporationskoderna vars teori beskrivs i \ref{appendixincltheory}.
