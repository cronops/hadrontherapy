...
In this simulation the beam is reproduced at the beam source (see ~\ref{fig:SimSetup}) as measured by Haettner with no phantom and zero distance from start detector to MWPC-detector. This means that all scattering will have been produced by the air in the IC or by the aluminium foil window. The beam will thus at its origin in the simulation have an FWHM of 4mm as per E.Haettner 4.7. A gaussian FWHM of 4mm is equivalent to a standard deviation of 9.42mm according to the relation $\mathrm{FWHM} =   2 \sqrt{2 \ln 2 } \; \sigma \approx 2.35482 \; \sigma$ . %perhaps the mwpc should be accounted for or simulated?
