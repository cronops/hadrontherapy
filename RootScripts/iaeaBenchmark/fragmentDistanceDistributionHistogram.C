...
void fragmentDistanceDistributionHistogram() {
gStyle->SetOptStat(0000000000); //remove the redundant statbox
 ...
 //this will be used as base for pulling the experimental data
   ...
   //TString simulationDataPath = "IAEA_34.7.root";
	...
	*/
	Double_t maxDistance = 5.0; //highest plotted distance
	int binAmount = 30; //amount of bins in plotted histogram
    TH1F *hist1 = new TH1F("hist1", "Fragment distance distribution", binAmount, 0.0, maxDistance); //The histogram for the angular distribution at a set length
	//This histogram needs to be hist1 with a symmetric negative side
	TH1F *symmetricHist = new TH1F("symmetricHist", "Fragment angular distr.", 2*binAmount, 0, .5); //bin amount must be even
...
//Projection from ntuple to histogram, so that the angle is trigonometrically pulled as the binned variable
	//fragments->Project("hist1","sqrt(posZ^2+posY^2)", "(Z == " + Znum + " && posY < .5 && posY > -.5)");
fragments->Project("hist1","sqrt(posZ^2+posY^2)", "(Z == " + Znum + ")");
...
for(int bin = 0; bin <= hist1->GetNbinsX(); bin++){
		...
		Double_t center = hist1->GetBinCenter(bin);
		Double_t low = center - width/2;
		Double_t high = center + width/2;
		Double_t squareTerm = (high*high) - (low*low);
		Double_t normArea = TMath::Pi()*4*squareTerm;
		deltaPhi = width/2;
		hist1->SetBinContent(bin, value/normArea); //Solid angle and amount of events
	}

	symmetricHist->SetAxisRange(0,12);
	//symmetricHist->SetXTitle("Distance (cm)");
