...
void fragmentAngularDistributionCompareAnnulusAndStraight() {
 ...
 //this will be used as base for pulling the experimental data
   ...
   metadata->GetEntry(0); //there is just one row to consider.
	...
	TNtuple* distrib2 = new TNtuple("angularDistrib","FragmentAngularDistrib","angle:particleAmount:normalized");
	...
	*/
	...
	Double_t normEntries = fragments->GetEntries("(Z == " + Znum + " && posY < 4.0 && posY > 0 &&  posZ > -" + rMaxString + " && posZ < " + rMaxString + ")");
...
for(Double_t j = 0.0; j <= 15.0; j=j+.1){
	degrees = j * TMath::DegToRad();
	rMinString = Form("%f", TMath::Tan(degrees)*scatteringDistance - detectorSideLength/2);	
	rMaxString = Form("%f", TMath::Tan(degrees)*scatteringDistance + detectorSideLength/2);	
	Double_t normEntries = fragments->GetEntries("(Z == " + Znum + " && posY < 2.0 && posY > -2.0 &&  posZ > " + rMinString + " && posZ < " + rMaxString + ")");
	Double_t zeroSA = 4 * TMath::ASin(pow(detectorSideLength,2.0) / (4*pow(scatteringDistance/TMath::Cos(degrees),2) + pow(detectorSideLength,2)) );
	Double_t zeroYieldNormed = normEntries / (events * zeroSA);

distrib2->Fill(-j,normEntries,zeroYieldNormed); //< degrees, entyamount, normalized result for graph
distrib2->Fill(j,normEntries,zeroYieldNormed); //< degrees, entyamount, normalized result for graph
}
	...
	distrib2->SetMarkerColor(kBlack);
	distrib2->SetMarkerStyle(2); //filled dot
	...
	distrib2->Draw("normalized:angle","angle > -3 && angle < 14","p,same"); //similar axises to e.haettner
